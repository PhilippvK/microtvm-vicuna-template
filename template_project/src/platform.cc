...
#ifndef VICUNA_CPU_FREQ_HZ
// Default: 100MHz
#define VICUNA_CPU_FREQ_HZ (100000000)
#endif  // VICUNA_CPU_FREQ_HZ
...
tvm_crt_error_t TVMPlatformMemoryAllocate(size_t num_bytes, DLDevice dev, void** out_ptr) {
  ...
  TVMLogf("Alloc: %u\n", num_bytes);
#endif
  auto ret = memory_manager->Allocate(memory_manager, num_bytes, dev, out_ptr);
  return ret;
}
...
tvm_crt_error_t TVMPlatformTimerStop(double* elapsed_time_seconds) {
  ...
  *elapsed_time_seconds = (microtvm_stop_time - g_microtvm_start_time) / (float)(VICUNA_CPU_FREQ_HZ);
#ifdef PRINT
